...
while running:
    ...
    cv2.putText(annotated_frame, f"FPS: {round(fps)}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 2)
    cv2.putText(annotated_frame, f"X: {round(x)}", (400, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (0, 0, 255), 1)
    cv2.putText(annotated_frame, f"Y: {round(y)}", (400, 140), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (0, 0, 255), 1)
    cv2.putText(annotated_frame, f"D: {round(distance)}", (400, 210), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (0, 0, 255), 1)
    cv2.putText(annotated_frame, f"A: {round(yaw)}", (400, 280), cv2.FONT_HERSHEY_SIMPLEX, 1.6, (0, 0, 255), 1)
    
    count = 0
    print(object_locations)
    for obj in object_locations:
        cv2.putText(annotated_frame, f"{count + 1}: {obj} {round(object_locations[obj][0]), round(object_locations[obj][1])}", (20, 160 + count * 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 9, 0), 1)
        count += 1    
